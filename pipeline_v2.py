...
all_numeric_features = (
    ...
    trend_features + breach_history_features + chemistry_features
)
...
print("Saved preprocessor.pkl and inference_config.json")
...
model_visit.save_model(os.path.join(OUTPUT_DIR, 'xgb_visit_timing.json'))
print("  Saved xgb_visit_timing.json")
...
for target_name, target_col in [
    ...
    model.save_model(os.path.join(OUTPUT_DIR, f'xgb_{target_name}.json'))
...
report = []
report.append("=" * 70)
report.append("  POOL PREDICTIVE MAINTENANCE V2 — EVALUATION REPORT")
...
report.append(f"Readings with ops data:         {pct_ops:.1f}%")
report.append(f"Readings with products data:    {pct_prod:.1f}%")
...
report.append("-" * 40)
report.append(f"Regulatory framework:           RD 742/2013 (national) + D 85/2018 (Valencia)")
report.append(f"  pH range:                     {REG_PH_MIN} – {REG_PH_MAX}")
report.append(f"  Free chlorine ideal range:    {REG_CHLORINE_MIN} – {REG_CHLORINE_IDEAL_MAX} mg/L (overdosing up to {REG_CHLORINE_CLOSE} is common)")
report.append(f"  Turbidity max:                {REG_TURBIDITY_MAX} NTU")
report.append(f"  Pool closure threshold:       Cl < {REG_CHLORINE_MIN} or > {REG_CHLORINE_CLOSE} mg/L")
report.append(f"")
report.append(f"Breach rate (at next visit):     {breach_pct_overall:.1f}%")
report.append(f"  pH breaches:                  {df_model['ph_breach_next'].sum()} ({100*df_model['ph_breach_next'].mean():.1f}%)")
report.append(f"  Chlorine breaches:            {df_model['chlorine_breach_next'].sum()} ({100*df_model['chlorine_breach_next'].mean():.1f}%)")
report.append(f"  Turbidity breaches:           {df_model['turbidity_breach_next'].sum()} ({100*df_model['turbidity_breach_next'].mean():.1f}%)")

report.append("\n\n3. FEATURE NULL RATES")
report.append("-" * 40)
for feat, rate in feature_null_pct.sort_values(ascending=False).items():
    if rate > 0:
        report.append(f"  {feat:40s} {rate:6.1f}%")

report.append("\n\n4. MODEL PERFORMANCE")
report.append("-" * 55)
report.append(f"{'Model':<20} {'RMSE':>8} {'MAE':>8} {'R²':>8} {'Trees':>8}")
report.append("-" * 55)
for name, res in results.items():
    unit = ' days' if name == 'visit_timing' else ''
    report.append(f"{name:<20} {res['rmse']:8.4f} {res['mae']:8.4f} {res['r2']:8.4f} {res['best_iter']:>8}")

report.append("\n\n5. TOP 5 SHAP FEATURES PER MODEL")
report.append("-" * 40)
...
report.append("\n\n6. EXAMPLE PRESCRIPTIONS (Visit Timing + Chemical Dosage)")
report.append("-" * 40)
...
expected_files = [
    'master_dataset.csv',
    'xgb_visit_timing.json',
    'xgb_ph.json',
    'xgb_chlorine.json',
    'xgb_turbidity.json',
    'shap_summary_visit_timing.png',
    'shap_summary_ph.png',
...
print(f"\n{'='*70}")
print("  PIPELINE V2 COMPLETE — Visit Timing + Chemical Dosage")
