...
export default function AdminPage() {
  ...
  const retrainMut = useMutation({
    ...
    onSuccess: (d) => {
      setMsg({
        type: "success",
        text: `Retrain completed: ${d.result?.run_id || ""} (Status: ${d.result?.status || d.status})`,
      });
      queryClient.invalidateQueries({ queryKey: ["admin-runs"] });
      queryClient.invalidateQueries({ queryKey: ["fleet"] });
    },
    onError: (e: Error) => setMsg({ type: "error", text: `Retrain failed: ${e.message}` }),
  });

  const weatherMut = useMutation({
    mutationFn: () => api.admin.weather(),
    onSuccess: (d) => {
      setMsg({ type: "success", text: `Weather synchronized: ${d.rows_upserted} days updated in database.` });
      queryClient.invalidateQueries({ queryKey: ["admin-wx-status"] });
    },
    onError: (e: Error) => setMsg({ type: "error", text: `Weather refresh error: ${e.message}` }),
  });
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold tracking-tight text-[#e8eaed]">System Administration & Governance</h2>
        <p className="text-xs text-[#9aa0a6]">Model lifecycle management, Open-Meteo weather intelligence, and ingestion audit logs</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-[#1a1d27] border border-[#2d3141] rounded-xl p-4">
          ...
          <div className="text-[10px] text-green-400 mt-1">✓ Prisma ORM Connected</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-8">
        <button
          ...
          className="p-5 bg-[#1a1d27] border border-[#2d3141] hover:border-[#4f8ff7] rounded-xl text-left transition disabled:opacity-50 group"
        >
          <div className="flex items-center justify-between mb-1">
            <span className="text-base font-semibold text-[#e8eaed] group-hover:text-[#4f8ff7] transition">
